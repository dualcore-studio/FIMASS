...
import { isAppointmentClosed, modalitaBadgeClass, modalitaLabel } from '../../utils/appointmentLabels';
...
function FornitoreDetailForm({ appointment, onClose, onRefresh, onError, onSuccess, suppliers }: FormProps) {
  const { user } = useAuth();
  const [draftLuogo, setDraftLuogo] = useState(() => appointment.luogo ?? '');
  const [draftLink, setDraftLink] = useState(() => appointment.link_videocall ?? '');
  const [histOpen, setHistOpen] = useState(false);

  const canEditConfirmFields =
    user?.role === 'fornitore' &&
    Number(appointment.fornitore_id) === Number(user.id) &&
    !isAppointmentClosed(appointment.stato) &&
    (appointment.stato === 'RICHIESTO' || appointment.stato === 'DA RIPROGRAMMARE');
    ...
    <>
      <div className="space-y-4">
        ...
        {appointment.modalita === 'presenza' ? (
          <div>
            <label htmlFor="appt-luogo-overview" className="text-sm font-medium text-slate-700">
              Luogo {canEditConfirmFields ? <span className="text-red-600">*</span> : null}
            </label>
            {canEditConfirmFields ? (
              <input
                id="appt-luogo-overview"
                className="input-field mt-1 w-full text-sm"
                value={draftLuogo}
                onChange={(e) => setDraftLuogo(e.target.value)}
        ...
        {appointment.modalita === 'videocall' ? (
          <div>
            <label htmlFor="appt-vc-overview" className="text-sm font-medium text-slate-700">
              Link videocall {canEditConfirmFields ? <span className="text-red-600">*</span> : null}
            </label>
            {canEditConfirmFields ? (
              <input
                id="appt-vc-overview"
              ...
              <p className="mt-1 break-all rounded-lg border border-slate-200/80 bg-slate-50/80 px-3 py-2 text-sm">
