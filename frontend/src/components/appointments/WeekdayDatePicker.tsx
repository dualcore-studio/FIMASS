...
import { dataIsoIsWeekdayMonFri } from '../../utils/appointmentVideocallTelefonataSlots';
...
/** Solo lunedì–venerdì; sabato e domenica disabilitati. Formato trigger DD/MM/AAAA. */
export default function WeekdayDatePicker({
  ...
  useEffect(() => {
    if (value && dataIsoIsWeekdayMonFri(value)) {
  ...
  const triggerLabel =
    selected && dataIsoIsWeekdayMonFri(value) ? format(selected, 'dd/MM/yyyy') : placeholder;

  const calendarPanel = open && !disabled && (
    <div
      ref={panelRef}
      role="dialog"
      aria-label="Calendario: giorni feriali"
