...
const { runWithRequestCache } = require('./data/store');
...
function createApp() {
  ...
  // Ogni richiesta ha la sua cache di lettura: le tabelle lette più volte dallo
  // stesso handler costano una query sola. Vedi `data/store.js`.
  app.use((req, res, next) => runWithRequestCache(() => next()));
