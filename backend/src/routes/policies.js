...
async function policyHasPolizzaEmessaAttachment(policyId) {
  const attachments = await list('attachments');
  return attachments.some(
    (a) =>
      a.entity_type === 'policy' &&
      Number(a.entity_id) === Number(policyId) &&
      a.tipo === 'polizza_emessa',
  );
}
  ...
  (async () => {
    ...
    try {
      ...
      if (alert === 'stale_policies') {
        ...
        policies = policies.filter(
          (p) =>
            ['RICHIESTA PRESENTATA', 'IN EMISSIONE'].includes(p.stato) &&
            String(p.updated_at || '') <= threshold,
        );
      }
...
router.get('/stats', authenticateToken, (req, res) => {
  (async () => {
    try {
      ...
      stati.forEach((s) => {
        stats[s] = policies.filter((p) => p.stato === s).length;
      });
      stats.totale = policies.length;
      res.json(stats);
    } catch (err) {
  ...
  (async () => {
    try {
      ...
      const history = ctx.policy_status_history
        ...
        .map((h) => ({
          ...h,
          stato_precedente: h.stato_precedente ? normalizePolicyStato(h.stato_precedente) : null,
          stato_nuovo: normalizePolicyStato(h.stato_nuovo),
          ...ctx.usersById.get(Number(h.utente_id)),
        }))
      ...
      if (req.user.role === 'struttura' && policy.stato !== 'EMESSA') {
...
router.put('/:id/status', authenticateToken, authorizeRoles('admin', 'supervisore', 'operatore', 'fornitore'), (req, res) => {
  (async () => {
    ...
    if (stato === 'EMESSA') {
      const hasFile = await policyHasPolizzaEmessaAttachment(req.params.id);
      if (!hasFile) {
        return res.status(400).json({
          error: 'Impossibile impostare EMESSA senza il file della polizza emessa allegato.',
        });
      }
    }
